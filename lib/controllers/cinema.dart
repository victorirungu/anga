...
class CinemaController extends GetxController {
  ...
  RxList scheduleFilter = ['All Films'].obs;
  RxList filteredScheduleFilms = [].obs;
...
//SCHEDULE
  ...
  updateScheduleFilter(String text) {
    text = removeAnga(text);
    if (text == 'Full Schedule') {
      text = 'All Films';
    }

    if (text.trim() != "All Films") {
      if (scheduleFilter.contains('All Films')) {
        scheduleFilter.remove('All Films');
      }
    } else {
      scheduleFilter.clear();
    }

    scheduleFilter.contains(text)
        ? scheduleFilter.remove(text)
        : scheduleFilter.add(text);
  }

  String removeAnga(String input) {
    final pattern = RegExp(r'\bANGA\s*', caseSensitive: false);
  ...
  getFilteredItems() {
    final normalizedFilter =
        scheduleFilter.map((filter) => filter.trim().toLowerCase()).toList();

    if (normalizedFilter.contains('all films')) {
      return schedule;
    } else {
      return schedule.where((item) {
        final experience = item['experience'].toString().trim().toLowerCase();
        final location = item['location'].toString().trim().toLowerCase();
        return normalizedFilter.contains(experience) ||
            normalizedFilter.contains(location);
      }).toList();
    }
  }
}
